...
import { getAigramUserName } from './aigram.js';
...
async function main() {
    ...
    if (!restored) seedExampleVillage(game);

    loadingScreen.classList.add('hidden');
    app.classList.remove('hidden');

    // Greet by name if Aigram tells us who's playing; otherwise a generic
    // welcome. We don't gate the reveal on this — the lookup races the
    // animation and lands as soon as the host responds.
    getAigramUserName().then(name => {
        if (restored) {
            ui.showToast(name ? `Welcome back, ${name}` : 'Welcome back');
        } else if (name) {
            ui.showToast(`Welcome to your isle, ${name}`);
        }
    });
}
